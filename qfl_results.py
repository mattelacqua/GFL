...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("USAGE: {python3 | pypy3}", sys.argv[0], "learning-time num-games num-tests")
        sys.exit(1)
    ...
    try:
        ...
        print("USAGE: {python3 | pypy3}", sys.argv[0], "learning-time num-games num_tests")
        sys.exit(1)
    ...
    for model_no in range(0,2):
        for grid_size in range(3, 6):
            ...
            for x in range(0, iterations):
                ...
                start = time.time();
                policy = qfl.q_learn(model, limit, grid_size);
                t = time.time() - start
                if t > limit:
